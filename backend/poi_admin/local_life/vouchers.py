...
class VoucherService:
    ...
    async def upsert_remote_voucher(
        ...
        *,
        ...
        voucher = (
            await self.session.execute(
                select(LocalVoucher).where(
                    ...
                    LocalVoucher.external_voucher_id == result.external_id,
                )
            )
        ).scalar_one_or_none()
        if voucher is None:
            voucher = LocalVoucher(
                ...
                order_id=order_id,
                external_voucher_id=result.external_id,
            )
        ...
        voucher.external_sku_id = _str_or_none(raw.get("sku_id"))
        voucher.code_masked = mask_voucher_code(_raw_code(raw))
        voucher.state = result.state
        voucher.consume_store_id = result.consume_store_id or _str_or_none(
            raw.get("consume_store_id")
        )
        voucher.valid_from = _parse_dt(raw.get("valid_from"))
        voucher.valid_until = _parse_dt(raw.get("valid_until"))
...
def voucher_operation_handlers(
    ...
    *,
    ...
    async def consume(operation: IntegrationOperation) -> dict[str, Any]:
        ...
        if not isinstance(voucher_id, str) or not isinstance(out_store_id, str):
        ...
        try:
            result = await gateway.consume_voucher(
                voucher.external_voucher_id, out_store_id=out_store_id
            )
            ...
            if isinstance(error, GatewayTransientError) and error.code not in {
                ...
                raise
            remote = await gateway.get_voucher(voucher.external_voucher_id)
    ...
    async def revoke(operation: IntegrationOperation) -> dict[str, Any]:
        ...
        try:
            result = await gateway.revoke_consumption(
                ...
                out_store_id=out_store_id if isinstance(out_store_id, str) else None,
            )
        except (GatewayTransientError, TimeoutError) as error:
            if isinstance(error, GatewayTransientError) and error.code not in {
                "timeout",
