...
export function ProductsPage() {
  ...
  return <section className="workspace-page">
    ...
    <Modal title="新建团购商品" open={open} onCancel={() => setOpen(false)} footer={null} destroyOnHidden>
      <Form form={form} layout="vertical" initialValues={{ sale_price: 9900, market_price: 12900, stock: 10 }} onFinish={values => create.mutate({ connection_id: values.connection_id, idempotency_key: 'product-create:' + Date.now(), merchant_product_id: values.merchant_product_id, name: values.name, head_images: [values.head_image], skus: [{ merchant_sku_id: values.merchant_sku_id, name: values.sku_name, sale_price: values.sale_price, market_price: values.market_price, stock: values.stock }] })}>
