...
export function WebhooksPage() {
  ...
  return <section className="workspace-page">
    ...
    <Card className="workspace-table-card" variant="borderless"><WorkspaceState loading={query.isPending} error={query.error ?? retry.error} empty={!rows.length}>
      <Table<WebhookRow> rowKey="id" dataSource={rows} pagination={{ pageSize: 10 }} columns={[{ title: '事件类型', dataIndex: 'event_type' }, { title: '状态', dataIndex: 'status', render: value => <Tag color={value === 'processed' ? 'green' : value === 'failed' ? 'red' : 'blue'}>{value}</Tag> }, { title: '尝试次数', dataIndex: 'attempt_count' }, { title: '接收时间', dataIndex: 'received_at' }, { title: '错误', dataIndex: 'error_message', ellipsis: true, render: value => value || '-' }, { title: '操作', render: (_, row) => editable && row.status === 'failed' && <Button type="link" icon={<RotateCcw size={14} />} loading={retry.isPending} onClick={() => retry.mutate(row.id)}>重新处理</Button> }]} />
    </WorkspaceState></Card>
  </section>
}
