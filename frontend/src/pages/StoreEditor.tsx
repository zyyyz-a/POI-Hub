...
export function StoreEditor() {
  ...
  return <>
    <Button type="primary" icon={<Plus size={15} />} onClick={() => setOpen(true)}>新建门店</Button>
    <Modal title="新建门店" open={open} onCancel={() => setOpen(false)} footer={null} destroyOnHidden>
      <Form form={form} layout="vertical" initialValues={{ status: 'active' }} onFinish={values => mutation.mutate(values)}>
        ...
        <Button block type="primary" htmlType="submit" loading={mutation.isPending}>创建门店</Button>
      </Form>
    </Modal>
  </>
}
